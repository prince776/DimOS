...
class VMM {
  ...
  public:
    ...
    /**
     * @brief Steps:
     * Do HHDM(Higher Half Direct Mapping 0xffff8...) for used up parts of
     * Physcial Memory Do KH(kernel Half) mapping for kernel Now VMM is gonna
     * have modes, user/kernel for kernel mode (only mode for now), VMM will
     * start looking for free pages starting from KH (2nd last PDE) For ease of
     * impl, we can try to map 2GiB of physical mem to KH, that puts a strict
     * req on memory though, even though kernel would probably need much less in
     * the foreseeable future. This also saves the trouble of keeping virtual
     * address of PageTables, so that'd cut our Paging Table cost by 50%
     */
    void init(const PhysicalMemMap& physicalMemMap, const MemRange* otherRanges,
